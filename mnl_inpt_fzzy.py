from thefuzz import process  # Importing the fuzzy matching tool
...
while True:
    ...
    # extractOne returns the best match and its similarity score (0-100)
    match_result = process.extractOne(user_input.lower(), medicines.keys())
    
    if match_result:
        best_match, score = match_result
        
        # If the score is high (e.g., above 80), we assume it's a match
        if score == 100:
            price = medicines[best_match]
            print(f"The price of {best_match.capitalize()} is ${price}")
        elif score >= 75:
            # If it's a close match but not perfect, we ask for confirmation
